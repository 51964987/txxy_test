#!/usr/bin/env bash
# ============================================================
# txxy 一键部署脚本 —— 环境 C: 其他 Linux（Ubuntu/Debian/CentOS 等）
# 用法:  bash deploy_linux.sh   （或 chmod +x 后 ./deploy_linux.sh）
...
if ! command -v docker >/dev/null 2>&1; then
    echo "[错误] 未检测到 docker，请先安装（Ubuntu/Debian: sudo apt install -y docker.io docker-compose-plugin）" >&2
    exit 1
fi
...
if ! docker compose version >/dev/null 2>&1; then
    echo "[错误] 缺少 docker compose 插件，请安装 docker-compose-plugin（或旧版用 docker-compose）" >&2
    exit 1
fi
