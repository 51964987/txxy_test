...
16. **同样的问题必须举一反三**：发现一个 bug 后，不能只修用户报的那一个点，必须主动排查**同类问题是否存在于其它代码路径**并一并处理：
    ...
    - **本项目实例**：用户报「进度 50/50 但明细全是已取消」，根因是 `done` 计数口径错误；举一反三后另查出 `_load` 对崩溃任务的 `running` 项不清理（永久卡「正在下载」）、明细「重新下载」按钮 tooltip 文案与实际行为不符、`running/pending` 行可误点等 3 个问题，一并修复。
...
12. **前端修复默认兼容移动端 + 全流程实测才交付**：涉及前端展示页面的改动，除非用户明确要求「仅桌面端」，默认都要一并兼容修复移动端——响应式布局、触摸手势、窄屏错位、分页与按钮在手机上的可达性等都需覆盖，不能「桌面正常即可」。交付前必须按真实链路端到端实测通过：覆盖桌面与移动两种视口，验证点击 / 滑动 / 缩放 / 翻页 / 关闭等关键交互在两种环境下均无异常；移动端可用浏览器设备模拟（DevTools 设备工具栏）或真实窄屏窗口验证。发现问题当场修复，禁止以「桌面正常」带病交付（与第 15 条「修复必须实测通过再交付」同源，本条进一步锁定移动端口径）。
