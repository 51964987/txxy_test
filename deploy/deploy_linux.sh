#!/usr/bin/env bash
# ============================================================
# txxy 一键部署脚本 —— 其他 Linux（Ubuntu/Debian/CentOS 等，联网环境）
# 用法:  bash deploy/deploy_linux.sh [--shared-db]
#   默认（不加参数）: 数据使用命名卷，与宿主机目录隔离
#   --shared-db      : 共用宿主机 ./db ./outputs ./downloads（bind mount）
#
# 离线（无外网）环境请用 deploy_offline.sh。
#
# 定时抓取默认不启动（cron 在 profiles 内），需要时执行:
#   docker compose --profile cron up -d --build
#
# 前置:  已安装 docker 与 docker compose 插件, 且 docker 已启动:
#        Ubuntu/Debian: sudo apt install -y docker.io docker-compose-plugin
#        CentOS/RHEL :  yum install -y docker-ce docker-compose-plugin（需先配官方 repo）
...
if ! command -v docker >/dev/null 2>&1; then
    echo "[错误] 未检测到 docker（Ubuntu/Debian: sudo apt install -y docker.io docker-compose-plugin）" >&2
    exit 1
fi
if ! docker info >/dev/null 2>&1; then
    echo "[错误] Docker 守护进程未运行，请执行: sudo systemctl enable --now docker" >&2
    exit 1
fi
if ! docker compose version >/dev/null 2>&1; then
    echo "[错误] 缺少 docker compose 插件，请安装 docker-compose-plugin" >&2
    exit 1
fi
...
# 宿主机映射端口：本环境（Linux）默认 18088；已显式填写则保留用户值
