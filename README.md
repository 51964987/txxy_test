...
- **页面**：
  - 数据总览（分区加载：P0 首屏、P1 视口懒加载 `IntersectionObserver`）：
    ...
    - **采集健康条**（P0）：位于 KPI 卡上方，三态横幅（绿/橙/红）+ 一句话结论（抓取中进度 / 已 N 天无入库 / 最近批次 N 个版块失败 / 入库正常 N/N），由 `/api/stats/health` 提供，判定口径（`run_lag_days` ≥2 升 warn、≥3 升 danger；空窗 vs 抓取中断优先级；**手动中断 cancelled 亦判 warn（非完成态不再标绿）**）收敛在后端 `_health_verdict` 一处，前端只上色不各判一套阈值；悬浮展示批次日期/耗时/版块成功率/最新发布日，点击跳 `/runs`；抓取中时圆点脉冲（与 B1 抓取中徽标同节奏）；
    ...
    - **内容资产漏斗**（P0）：紧凑横条卡四级「收录帖子 → 已下载帖 → 本地文件 → 占用体积」，首级箭头附转化率（已下载帖 / 收录帖子），四级配色沿用趋势统计卡家族色（蓝/绿/紫/橙，不另建色板），由 `/api/stats/assets` 驱动（口径与资源管理页一致：`download_tasks` 中已落盘且文件仍在的 URL 与 posts 取交集）；右侧「资源管理」入口跳 `/resources`；卡内新增「按类型」占比条（图片/视频/种子/磁力链接/云盘清单/文本/其他，按体积占比着色，复用资源管理页同款分类与色板；占比统一保留 2 位小数），点击任一类下钻到资源管理页按该类型筛选（继承类型上下文，数字自洽）；已下载帖与本地文件全为 0 时（任务 + 文件 + 履历全部清空）渲染「尚未下载任何内容」引导空态（含去资源管理入口），而非生硬的「0 漏斗」；
    - **每日发布趋势（双图并排）**：左「全站发布趋势」+ 右「分版块发布对比」，天数 7/14/21/28 自动轮播（下拉可切换/自定义输入），两图双向 Tooltip 联动聚焦；卡头统计卡（峰值/谷值/日均、对比版块数/最活版块/峰值日增）+ 折线 CSS 流光 + 数字滚动；全站趋势卡头新增「近 7 日环比 `↑xx%`」统计卡（色随涨跌，由 `/api/stats/compare.week.delta` 驱动，与分板块 7 日环比同源同窗，悬浮提示口径；2026-09-11 由 30 日统一，见 §19），分板块卡头「最活板块 7 日环比」（由 `/stats/trend_by_fid` 每系列 `delta` 驱动，与活跃榜 7 日环比同源同窗），分板块 tooltip 每行尾附「· 7日 ↑/↓x%」环比，>31 点启用 DataZoom；
    - **活跃作者 Top10 / 活跃版块 Top10（左右 1:1 横向条形图）**：按累计发帖量排序（`/api/stats/top_authors` / `/api/stats/top_fids`）；作者条点击下钻帖子浏览（按作者精确过滤），版块条点击跳该版块列表；
    - **热门榜（P1 懒加载，4 栏）**：点赞最高帖 / 回复最高帖（`/api/stats/boards`）、最新最热（`/api/stats/today_top`，最新数据日期内点赞+回复综合）、本月最热（`/api/stats/month_top`）；点赞·回复最高帖**标题点击 `window.open` 原帖 URL**，**卡片空白区点击**下钻该版块（`/posts?fid=&sort=likes_desc|replies_desc`），**「查看更多」**带排序下钻帖子浏览（`/posts?sort=likes_desc` / `/posts?sort=replies_desc`）；每行悬浮「下载」按钮直接创建下载任务（与帖子浏览同一套判重交互：防连点、正在下载中的剔除、已下载过的确认后提交；真全屏态先退出全屏再提示；进度在下载中心查看）；
    - **待下载推荐**（P1）：热门榜下方新区块，由 `/api/stats/pending_downloads?limit=8&days=30` 驱动——窗口内互动量最高、且尚未下载到本地的帖子，已下载判据为**下载履历 ∪ 任务历史**（清空任务中心不会导致重复推荐，见下载中心「下载履历」说明），下载中亦自动排除；若帖子曾被下载但文件已被资源管理清空，则标「可重下」橙色标签区别于全新待下载（复用 `gone_urls()` 判据）；桌面双列、窄屏单列；标题打开原帖、行内下载按钮复用热门榜同一交互；**行不可下钻**（帖子页无「未下载」过滤条件，行内下钻必然口径不一致；与既有「下钻必须继承口径」约定同源）；每轮自动刷新（5s）——下载完成后相应帖子自动退出推荐位；卡头右侧「查看全部 ›」下钻到帖子页（`/posts?date_from&date_to&undownloaded=1&sort=engagement_desc`），继承「近30日 · 未下载 · 按互动量」上下文，列表为全量明细（数字自洽，顶部摘要条可逐项清除）；
    - 布局：趋势双图 `1fr 1fr`、活跃榜双视图 `1fr 1fr`、热门榜 4 栏等宽，间距统一 16px；窄屏自动降级为单列；大屏态图表高度按视口三等分（`--chart-h`）、间距收紧，尽量一屏铺满；
  - 帖子浏览（`/posts`）：版块多选 / 日期区间 / 标题或作者关键词筛选，分页排序（发布日期/发布时间/点赞/回复），**顶部与底部双分页**（共用同一翻页状态，**单行满宽**——total 靠左、每页条数与翻页控件靠右；长列表滚到顶也能直接翻页；窄屏由全局规则收敛为 total+prev/pager/next），支持从数据总览下钻（带 `fid` / `author` / `sort` 预筛选，下钻作者回填关键词框）；操作栏图标按钮（悬浮「打开/复制链接/下载」），表格多选 + 一键「批量下载」到下载中心（提交前与下载中心页内提交同一套判重：正在下载中的链接自动剔除避免并发写同一文件，已下载过的先弹窗确认再提交），一键导出 CSV（10 列带 BOM，Excel 可直接打开，限 5 次/分）；
