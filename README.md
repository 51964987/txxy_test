...
- **页面**：
  - 数据总览（分区加载：P0 首屏、P1 视口懒加载 `IntersectionObserver`）：
    ...
    - **内容资产（沉淀进度，精简 KPI 摘要）**（P0，2026-09-13 重构；**2026-09-18 全量卡片合并至资源管理页**；**2026-09-18 细化**：摘要卡右上角统一显示「下钻 ›」跳资源管理（与「今日发布」同款弱化灰字 / hover 变蓝），「还差 N 帖」卡面常驻极小字标明所属目标档，如「互动Top500 内还差 N 帖」）：数据总览仅保留一行精简 KPI 摘要——「沉淀完成度% + 目标缺口」（当前沉淀率 vs 设置页目标档位，进度条 + 还差 N 帖），由 `/api/stats/assets` 驱动；完整 KPI 优先卡片（头条目标进度 + 三级漏斗 / 五态行 / 分层沉淀率 / 对账 / 存储行 / 增长柱，因与资源管理 B6 同源而**不含类型分布**）已整体并入资源管理页（详见《资源管理页面优化方案.md》内容资产卡合并节）；取数与派生指标抽离为 `composables/useAssets.ts` 单一实现，两页共用，禁止两处各写一遍；
