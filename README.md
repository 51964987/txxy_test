...
- **下载中心接口**（`/api/downloads`）：
  ...
  - `POST /api/downloads/{tid}/cancel`：取消任务——运行中任务标记取消标志（worker 在下一个链接前收手），排队中任务立即置为已取消终态并使队列令牌失效（不必等 worker 消费）；
  - `POST /api/downloads/{tid}/retry`：在原任务内重跑未成功项（fail/cancelled/遗留 running 重置为 pending 重新入队，成功项不重复下载，进行中任务拒绝，返回 `{retried}`）；
