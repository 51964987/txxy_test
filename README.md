...
- **页面**：
  - 数据总览（分区加载：P0 首屏、P1 视口懒加载 `IntersectionObserver`）：
    ...
    - **待下载推荐**（P1）：热门榜下方新区块，由 `/api/stats/pending_downloads?limit=8&days=30` 驱动——窗口内互动量最高、且尚未下载到本地的帖子，已下载判据为**下载履历 ∪ 任务历史**（清空任务中心不会导致重复推荐，见下载中心「下载履历」说明），下载中亦自动排除；若帖子曾被下载但文件已被资源管理清空，则标「可重下」橙色标签区别于全新待下载（复用 `gone_urls()` 判据）；桌面双列、窄屏单列；标题打开原帖、行内下载按钮复用热门榜同一交互；**行不可下钻**（帖子页无「未下载」过滤条件，下钻必然口径不一致；与既有「下钻必须继承口径」约定同源）；每轮自动刷新（5s）——下载完成后相应帖子自动退出推荐位；右侧「下载中心」入口跳 `/downloads`；
