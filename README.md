...
> 定位：**单人自用的「内容采集 + 本地媒体资产管理」一体化工具**；Web 看板是它的界面层，不是全貌。
> 完整定位与前提见 `CODEBUDDY.md` 的「项目定位」小节。

从 txxy（本地镜像 `127.0.0.1:1024`）批量抓取版块帖子列表，写入 SQLite 数据库与 CSV；支持下载帖子页内的图片（GIF/JPG 分类存放）、视频、种子，以及导出磁力链接与云盘链接清单；并提供 Web 展示端——数据总览看板、帖子浏览、运行记录、资源管理与回收站（保留期内可恢复）。

数据入库只存相对路径、域名按环境自适应，便于在本机 / Docker / 离线（air-gapped）环境之间搬迁。
...
- **页面**：
  - 数据总览（分区加载：P0 首屏、P1 视口懒加载 `IntersectionObserver`）：
    ...
    - **卡片头部统一模式（2026-09-13 起，数据总览全部卡片）**：**第一行 = 「标题 + 口径说明副标题」**（副标题一句话写清窗口 + 指标 + 排序依据，悬浮显示完整口径：口径细节、同源同窗关系、交互含义）+ 右侧操作/筛选（查看更多 / 排序切换 / 范围切换 / 全屏）；**第二行起才是内容**（统计卡行 → 图表 / 列表）。此前统计卡挤在标题行右侧——半宽卡靠 `flex-wrap` 折行兜底、全宽卡与标题同行，同一套卡片观感不一；统一后 11 张卡片（内容资产 / 全站发布趋势 / 分版块发布对比 / 每日互动量趋势 / 待下载推荐 / 活跃作者 / 活跃版块 / 点赞最高帖 / 回复最高帖 / 最新最热 / 本月最热）头部层级一致，「最新最热 / 本月最热」原先占位的日期角标合并进副标题（信息不丢、少一个元素）；
    - **采集健康条**（P0）：位于 KPI 卡上方，三态横幅（绿/橙/红）+ 一句话结论（抓取中进度 / 已 N 天无入库 / 最近批次 N 个版块失败 / 入库正常 N/N），由 `/api/stats/health` 提供，判定口径（`run_lag_days` ≥2 升 warn、≥3 升 danger；空窗 vs 抓取中断优先级；**手动中断 cancelled 亦判 warn（非完成态不再标绿）**；2026-09-13 新增**资产停滞**：采集正常但连续 14 天无新增本地资产亦升 warn，仅在 `level=ok` 时升级，不掩盖采集问题）收敛在后端 `_health_verdict` 一处，前端只上色不各判一套阈值；悬浮展示批次日期/耗时/版块成功率/最新发布日，点击跳 `/runs`；抓取中时圆点脉冲（与 B1 抓取中徽标同节奏）；
    - **KPI 统计卡片（4 张）**：累计收录（副指标：近 7 日发布 + 近 7 日环比 `↓xx%` + 覆盖版块数，环比由 `/api/stats/compare.week.delta` 驱动，与活跃榜 7d 滚动窗口同口径）、今日发布（副指标：较昨日环比 + 昨日）、发帖作者（副指标：今日更新人数 + 活跃率）、最近入库（最近批次日期 MM-DD + 批次运行时绿色脉冲「抓取中 N%」徽标 + 中性「更新于 HH:MM」；**停摆/批次失败告警已移交 R1 采集健康条，本卡不再重复呈现**）；由 `/api/stats/overview` + `/api/stats/compare` + `/api/runs` 驱动（`today_top` / `assets` 仍分别驱动热门榜与 R4 内容资产漏斗）；数据新鲜度以入库活动时间（`latest_run_at`，run_days 最近批次开始/结束时刻较大者）为准；栅格宽屏 4 列单行 / 中屏 2×2 / 窄屏单列（各端铺满不空旷）。
    - **内容资产（沉淀进度）**（P0，2026-09-13 重构）：紧凑横条卡，由 `/api/stats/assets` 驱动，共六行——① 三级漏斗「收录帖子 → 已沉淀帖 → 本地文件」（**同量纲计数**，体积不再作漏斗一级），首级箭头附全库转化率，`已沉淀帖` 可点击下钻 `/posts?downloaded=1`（与卡片数字严格自洽）、`本地文件` 跳 `/resources`；② **目标进度条**（设置页可配目标档位与覆盖率，默认「互动 Top500 / 50%」；**卡面按业界 stat-vs-target 形态收敛**——只留「沉淀目标 + 目标值小字 + 当前值 + 还差 N 帖」，档位名与「已下载 / 该档总数」明细分别交给④的目标档高亮与悬浮，「为何不下钻」的实现理由只留在代码注释与文档）；③ **状态行**：在途 / 失败 / 可重下 / 空壳 / 近 N 日缺口（缺口可点，下钻 `/posts` 的「近30日 · 未下载 · 互动量」全量明细）+ 右侧「对账 已认领/总目录」（悬浮给出磁盘目录 = 已认领 + 未认领 + 空壳的互斥拆分）；④ **分层沉淀率**：全库 / 互动≥10 / 互动 Top500 三档（**当前目标档高亮**——底色 + 左侧色条，作为②的分母口径唯一表达）+ 「集中在 X 版块」集中度（悬浮给全部版块明细）——全库百分比会被长尾稀释（实测 0.09% vs Top500 3.4%），必须分档看；⑤ **存储行**：占用 / 可用 / 总容量 + 近 N 日沉淀帖数与体积（纯 CSS 迷你增长柱；履历有时间戳的条目才计入，历史条目「时间不可考」时提示等新下载）；⑥ 既有「类型分布（按大小）」占比条 + 图例（图片/视频/种子/磁力链接/云盘清单/文本/其他，复用资源管理页同款分类与色板，占比 2 位小数），点击任一类下钻资源管理页按该类型筛选；已沉淀帖与本地文件全为 0 时（任务 + 文件 + 履历全部清空）渲染「尚未下载任何内容」引导空态；
    - **每日发布趋势（三图两行）**：上排左「全站发布趋势」+ 右「分版块发布对比」（双向 Tooltip 联动聚焦）；下排左**「每日互动量趋势」**（2026-09-13 从「本月最热」卡内 sparkline 抽出，见 §21）+ 右**「待下载推荐」**，两卡各占 1/2（左图看热度、右侧清单直接下载，形成「看到 → 行动」闭环；窄屏自动上下堆叠，卡片内清单同步单列，图表区按行高自适应不留白）；天数 7/14/21/28 自动轮播（下拉可切换/自定义输入），**三图同一窗口**（互动量与分版块均跟随全站趋势的天数，横轴严格对齐）；统计卡行（峰值/谷值/日均、对比版块数/最活版块/峰值日增、互动量窗口合计）位于标题行下方**独立一行**（三卡同构：标题+口径 → 统计行 → 图表，见「卡片头部统一模式」）+ 折线 CSS 流光 + 数字滚动；全站趋势卡头新增「近 7 日环比 `↑xx%`」统计卡（色随涨跌，由 `/api/stats/compare.week.delta` 驱动，与分板块 7 日环比同源同窗，悬浮提示口径；2026-09-11 由 30 日统一，见 §19），分板块卡头「最活板块 7 日环比」（由 `/stats/trend_by_fid` 每系列 `delta` 驱动，与活跃榜 7 日环比同源同窗），分板块 tooltip 每行尾附「· 7日 ↑/↓x%」环比，>31 点启用 DataZoom；**缩放选区联动（2026-09-13 修正为三图双向）**：三张趋势图**任一**拖动/滚轮缩放都会同步另外两张 —— 两张单指标图之间用 `dispatchAction('dataZoom')` 移视窗（按日期换算索引，不假设序列对齐），分版块图按该日期区间裁剪数据并挂「跟随趋势区间」徽标（**跨年时显示完整年月日**，365 天窗口跨年只取 MM-DD 会读成起止倒序）；分版块自身滑动条也可发起（其选区按「当前裁剪区间的实际日期」回传，满区间视为无变化，裁剪后自动复位为满宽），徽标 ✕ 一次复位三张图；**聚焦日联动（同日补齐为三图双向）**：三张图**任一**点击绘图区某列即聚焦该天，三图同时画「聚焦日」标线（单指标图由 `buildDayLineOption(markDate)` 画蓝色实线，分版块用既有 markLine），「联动聚焦日」徽标 ✕ 可清除 —— 判据不依赖折线命中（折线命中区仅 2px、且 series click 的 `dataIndex` 为 undefined），而是在 `getZr()` 层用 `containPixel({gridIndex:0})` 判定绘图区 + `convertFromPixel` 换算日期；**版块聚焦配色（分版块图例 → 全站线色）目前不可达**：该图例 `show: false`，待决策（见 §21.11）；**「今日未完整」口径**：窗口最后一天是今天（当日抓取未覆盖全天，实测发布量 322/前日 1007、互动量 1215/前日 12207），三图统一在该点画橙色虚线并标注，tooltip 追加说明，且**统计卡（峰值/谷值/日均/窗口合计）排除今天**——否则谷值永远落在今天；
    - **活跃作者 Top10 / 活跃版块 Top10（左右 1:1 横向条形图）**：按累计发帖量排序（`/api/stats/top_authors` / `/api/stats/top_fids`）；作者条点击下钻帖子浏览（按作者精确过滤），版块条点击跳该版块列表；
    - **热门榜（P1 懒加载，4 栏）**：点赞最高帖 / 回复最高帖（`/api/stats/boards`）、最新最热（`/api/stats/today_top`，最新数据日期内点赞+回复综合）、本月最热（`/api/stats/month_top`，**本栏只做列表**：卡内原先的每日互动量 sparkline 已于 2026-09-13 抽为趋势区「每日互动量趋势」独立图，见 §21；卡头保留「按数据最新月份统计 / 当月共 N 帖 / 覆盖 N 天」口径 tooltip、排序切换与「新入榜 NEW」标记）；点赞·回复最高帖**标题点击 `window.open` 原帖 URL**，**卡片空白区点击**下钻该版块（`/posts?fid=&sort=likes_desc|replies_desc`），**「查看更多」**带排序下钻帖子浏览（`/posts?sort=likes_desc` / `/posts?sort=replies_desc`）；每行悬浮「下载」按钮直接创建下载任务（与帖子浏览同一套判重交互：防连点、正在下载中的剔除、已下载过的确认后提交；真全屏态先退出全屏再提示；进度在下载中心查看）；
    - **待下载推荐**：位于「每日互动量趋势」同行右侧（各占 1/2，2026-09-13 由热门榜下方全宽卡移入，随首屏 P0 一起取数、无需滚动即加载），由 `/api/stats/pending_downloads?limit=10&days=30` 驱动——**滚动近 30 日窗口（含今天、会跨月）**内互动量最高、且尚未下载到本地的帖子，已下载判据为**下载履历 ∪ 任务历史**（清空任务中心不会导致重复推荐，见下载中心「下载履历」说明），下载中亦自动排除；卡头副标题与悬浮已显式写明窗口（「近30日滚动窗口（跨月）」）并说明**与「本月最热」的自然月窗口不同**（两者同按互动量排序，但窗口不同 → 榜单结果可能完全不重合，实测 Top8 零交集）；若帖子曾被下载但文件已被资源管理清空，则标「可重下」橙色标签区别于全新待下载（复用 `asset_snapshot().gone` 判据）；卡片内清单单列（因卡片自身已是半宽；窄屏堆叠后同样单列）；**列表沿用热门榜 `.board-list` 样式**（与「本月最热」同款）：取 **10 条**、超过 360px 即纵向滚动，细窄滚动条常显（`scrollbar-width: thin` + `::-webkit-scrollbar`，避免列表被截断却看不出「下面还有」）；标题打开原帖、行内下载按钮复用热门榜同一交互；**行不整体下钻**（卡头「查看全部 ›」已提供同口径全量明细入口；行内点击留给「下载」按钮与标题，避免整行点击与按钮互抢）；**提交下载后立即定向刷新**（2026-09-13）——点行内「下载」创建任务成功后，前端立即重拉本卡与「内容资产」卡，后端同步失效 `pending_downloads_*` / `assets_*` / `health_*` 统计缓存，该帖随即退出推荐位，不必等下一轮 5s 轮询（自动刷新被关闭时也不会再「点了下载、列表纹丝不动」）；**刻意不刷新榜单四卡**（点赞最高帖 / 回复最高帖 / 最新最热 / 本月最热）——它们按互动量排序、与「是否已下载」无关，重拉只会得到相同数据，反而打断列表滚动位置，见《数据总览大屏设计与优化总览.md》§21.12；卡头右侧「查看全部 ›」下钻到帖子页（`/posts?date_from&date_to&undownloaded=1&sort=engagement_desc`），继承「近30日 · 未下载 · 按互动量」上下文，列表为全量明细（数字自洽，顶部摘要条可逐项清除）；
    - 布局：趋势双图 `1fr 1fr`、活跃榜双视图 `1fr 1fr`、热门榜 4 栏等宽，间距统一 16px；窄屏自动降级为单列；大屏态图表高度按视口三等分（`--chart-h`）、间距收紧，尽量一屏铺满；
  - 帖子浏览（`/posts`）：版块多选 / 日期区间 / 标题或作者关键词筛选，分页排序（发布日期/发布时间/点赞/回复），**顶部与底部双分页**（共用同一翻页状态，**单行满宽**——total 靠左、每页条数与翻页控件靠右；长列表滚到顶也能直接翻页；窄屏由全局规则收敛为 total+prev/pager/next），支持从数据总览下钻（带 `fid` / `author` / `sort` 预筛选，下钻作者回填关键词框）：待下载推荐下钻带「未下载」筛选（`undownloaded=1`），内容资产卡下钻带「已下载」筛选（`downloaded=1`，与卡片「已沉淀帖」数字严格自洽），两者均在顶部摘要条集中展示、可逐项清除；操作栏图标按钮（悬浮「打开/复制链接/下载」），表格多选 + 一键「批量下载」到下载中心（提交前与下载中心页内提交同一套判重：正在下载中的链接自动剔除避免并发写同一文件，已下载过的先弹窗确认再提交；**在「未下载」筛选下提交成功后立即重刷当前页**——该行随即退出列表（本页被刷空时自动回退一页），见《数据总览大屏设计与优化总览.md》§21.12），一键导出 CSV（10 列带 BOM，Excel 可直接打开，限 5 次/分；**导出筛选条件与列表逐项一致**——含版块 / 日期 / 关键词 / 作者 / 高级条件 / 未下载 / 已下载 / 排序列，此前 `author` 与高级条件在导出侧被静默忽略，2026-09-13 修复）；
  ...
  - 资源管理：扫描 `downloads/` 目录，按文件夹分组展示文件清单与大小（**空目录也收录**，带「空目录」标记——多为下载失败/取消残留，可勾选批量清理；与「未下载到媒体」空壳标记互斥）；**全局搜索 / 类型筛选**（跨全部目录，命中片段高亮，命中文件带「所属目录」列，目录名命中可一键展开；命中结果**平铺单行列表 + 分页**（默认每页 30，可切 50/100；单行形态：文件名（类型色点）+ 大小 + **修改时间** + **所属目录/子路径** + 操作按钮（查看/浏览/打开/删除）一行铺开不换行，**操作区右对齐**（筛选模式下不重复显示类型标签，色点保留类型线索），移动端隐藏修改时间与所属目录、直显「浏览」+ 其余操作收进「更多」菜单；每行带统一「浏览」入口——按类型自动分派的「查看」与**跨目录浏览全部命中资源**的「浏览」（抽屉内与列表同一筛选口径，每页 30 可翻页看到后续页资源，目录头「浏览」仍为单目录模式）；刻意不提供「显示全部」防数千行卡死））；目录按时间 / 名称排序；目录列表**分页展示**（默认每页 30，可切 50/100 或「显示全部」），避免 145+ 个目录一次性铺开；目录头**单行形态**（名称省略 + 文件数·大小·时间 + 类型构成摘要 + 标记）、来源命中时操作区提供**原帖/看帖子**回溯（移动端收进「更多」菜单）、**下载任务关联**标记（跳转下载中心）与「未下载到媒体」空壳提示；所有文件行统一「查看」入口（按类型自动分派：图片大图查看器同列表连看 / 视频弹窗播放同列表连播、拖动进度走 HTTP Range / 文本与种子信息弹窗 / 其他用系统默认程序打开）；目录头「浏览」不展开目录即可网格浏览该目录全部资源（类型筛选 + 文件名过滤 + **排序**（类型优先/名称/大小/时间/尺寸）+ 前端分页每页 60；**图片缩略图受控并发加载**（并发 6 + 429 自动退避重试，错误态区分「限流中/加载失败」并给重试与本地打开入口，预览接口 300 次/分）；卡片显示**像素尺寸 · 大小**（后端零依赖解析文件头，不信任扩展名；真实格式标签已去除，避免与类型色点重复），悬浮显示修改时间与完整路径；汇总含「图片 N 张 / X MB」；点击卡片走同一查看入口）；**自主勾选 + 删除所选（文件与目录均支持）**（业界网盘通行做法：桌面表格勾选列含表头全选/半选、文件卡片与目录头带勾选框，文件选中跨目录累积、目录选中随刷新裁剪，工具栏「全选当前结果 / 取消全选」按模式分派——全局模式作用全部命中文件、目录模式作用当前页目录——+「删除所选（N）」混合整批提交；目录模式/全局搜索/类型筛选通用，三选确认：直接删除不可恢复 / 移入回收站）；文件行「删除」与目录头「删除目录」（**软删除**到 `outputs/trash/`，默认保留 7 天）；工具栏「回收站」抽屉可**恢复**或**彻底删除**（含清空，彻底删除需二次确认）；**容量洞察卡**（类型分布占比 / Top10 目录 / Top10 大文件，两卡片行操作与信息密度对齐：目录行支持浏览/原帖/删除，文件行支持查看/原帖/删除）；「打开」按钮调起资源管理器；搜索/筛选/排序/展开状态会话记忆；批量复制路径；加载失败重试与手动刷新（资源列表缓存 10 秒 TTL，感知 downloads/ 内非本应用产生的文件变化；限流：扫描 60 次/分、图片预览 60 次/分、视频播放 300 次/分、打开目录 10 次/分、删除类 10 次/分）；
  - 参数设置（`/settings`）：页内统一配置运行时参数（配置层级 **页内设置 > 环境变量 > 代码默认值**；仅白名单参数，端口/路径/域名等部署配置不在此处）。**下载**：单任务内并行下载数（1-8，下一任务生效）、任务间并行数（1-4，调大立即、调小在当前任务结束后收缩）、单次提交上限、任务历史保留条数、单文件下载间隔、请求重试次数与等待（后三者下一任务生效）；**资源与回收站**：回收站保留天数、资源扫描缓存 TTL（均立即生效）；**界面**：数据总览自动刷新（前端立即应用）；**内容资产**（2026-09-13 新增）：沉淀目标范围（全库 / 互动≥10 / **互动 Top500（推荐）**，单选枚举）与目标覆盖率（%），驱动数据总览资产卡的目标进度条与「还差 N 帖」（两项说明各一句，组级说明不重复，2026-09-13 文案收敛，见《数据总览大屏设计与优化总览.md》§20.6）。每项标注生效范围与默认值，支持单项/全部**恢复默认**；落盘到 `outputs/web_settings.json`（原子写，损坏回落默认），服务启动时自动应用已保存设置；
  ...
  - 下载中心（`/downloads`）：页内粘贴多行链接直接提交（自动拆解去重、无效行提示，重复提交前经 `check-dup` 接口二次确认），异步执行并以 **SSE 实时推送**进度（亚秒级，断线自动降级 3s 轮询），逐 URL 明细（含耗时）与任务日志在「详情」抽屉按需加载；**任务日志含完整下载过程**（正在请求 / 标题 / 保存目录 / 共提取到 N 个资源 / 逐文件 `[完成] xxx.jpg（字节数,链接）`，与 CLI 看到的一致），明细行以 `[序号/总数]` 前缀标注所属链接，最多保留最近 2000 条；**「按链接明细」折叠组按最后活动时间排序**（最新在最后，而非按链接序号——重下（行内「下载」/单条「重新下载」）只跑未成功链接、其序号往往更小，按序号排会让新输出落在列表中间），正在输出的组标「进行中」，配合自动跟随即 CI 日志的 tail -f 体验；状态筛选与汇总（进行中/已暂停/已完成/失败/已取消）、任务完成通知；任务列表按状态排序（进行中→排队中→已暂停→失败→已取消→已完成，已完成沉底），排队中任务按入队令牌升序展示（与实际执行顺序 FIFO 一致，置顶任务最前）、终态按创建时间倒序，并分页展示（前端切片，每页 20）；任务行可勾选（桌面表格与移动端卡片共用同一选中集合），工具栏**单一主操作按钮随在途状态自动切换**——无在途任务时是「开始下载」（对勾选的失败/已取消/已暂停任务批量下载：失败/已取消在原任务内重跑未成功链接，已暂停继续未完成链接，已在队列中/已完成/正在下载的跳过并给出原因），有在途任务时是「全部暂停」（暂停勾选的下载中任务，未勾选则暂停全部在途任务；**暂停是非终态**，未跑链接保留为 pending，可再「开始下载」继续，与「取消」严格区分）；行内「下载」即单任务版的同一入口（不另开任务、进度在原任务更新），排队任务可「优先执行」插队；详情抽屉内链接明细支持**单链接「重新下载」**（就地重跑原任务内该链接，不另开任务，结果仍显示在原详情页），明细区新增成功/失败/取消统计与状态筛选（全部/异常/成功）、异常项优先排序、固定高度内部滚动、分页展示（前端切片，每页 20、序号跨页连续）、「清空已完成」（只删已完成任务，失败/已取消保留；自动轮转仍按全部终态裁剪防膨胀）与历史自动轮转（默认保留 200 条，`TXXY_DOWNLOAD_TASK_MAX_KEEP`）；另有帖子浏览多选批量/单行下载、数据总览热门榜单行下载入口；后端 `download_tasks.py` 任务队列（任务间并发数 `TXXY_DOWNLOAD_TASK_CONCURRENCY` 默认 1 串行，队列支持插队），**任务收尾自动清理本任务遗留的空目录**（仅 `rmdir`，物理上不会误删有内容的目录），状态持久化到 `outputs/download_tasks.json`（`TXXY_DOWNLOAD_TASKS_FILE`，写盘前自动轮转一份 `.bak` 上一代备份，Web 进程不触碰 `posts.db`）；**下载履历**独立持久化到 `outputs/download_history.json`（`TXXY_DOWNLOAD_HISTORY_FILE`，结构 `url → {dir, first_at, checked_at, fail_at, fail_error}`，2026-09-13 由「url → 目录名」升维）：任务列表会被「清空已完成」与自动轮转裁剪，而「哪些帖子已下载过」是持久事实——履历与任务分开留存，判「已下载」= 履历 ∪ 任务历史中目录仍在磁盘者（提交前判重提示、数据总览「待下载推荐 / 内容资产 / 对账」共用同一口径）；新增的 `first_at`（首次落盘时间，只在缺失时写入）供资产增长曲线与沉淀速率使用，`fail_at` / `fail_error` 让**失败清单在任务面板被清空后依然留存**（此前履历只记成功）；每次启动从现存任务播种（`first_at` 取任务结束时刻近似）并按 `sanitize_title(库内标题) == 目录名` 从 `downloads/` 增量恢复（覆盖 CLI 直接下载的目录；**不再用「目录名 == 标题」全等判据**——目录名是标题经非法字符替换 + 截断 80 字符后的结果，长标题会永远匹配不上，2026-09-13 修复后实测认领数 130 → 132、未认领归零），清空任务中心不再丢失「已下载」事实；
- **资源管理接口**（`/api/resources`）：
  - `GET /api/resources`：扫描结果（目录分组 + 文件清单；增量缓存按顶层目录签名，限 60 次/分）；
  - `GET /api/resources/file?path=`：受控图片预览（限 300 次/分，仅图片白名单）；
  ...
  - `POST /api/resources/batch-delete`：`{ items:[{path, is_dir}], permanent }` 批量删除（单项失败不影响其余，单次上限 500；前端按 30 项/块分块提交并展示进度条；限流 60 次/分）；
...
- 文件分享（`/share`，独立 8090 端口，与主看板隔离）：资源管理「浏览」抽屉标题栏的「分享此目录」可把当前目录（经类型/名称筛选后）的全部文件合并成一个分享链接；卡片/行内「分享」仍支持单文件。分享页为画廊式预览（不含下载按钮），一个链接对应一组文件，有效期 1h/24h/7d/30d 可选；链接主机名可由页内「参数设置 → 分享链接」固定（局域网分享设本机 IP），否则自动沿用访问请求的 Host；分享索引写入 `outputs/share_index.json`（主应用写、分享服务只读），过期或任一文件已删除即 410。画廊页交互（参考业界图片集合浏览通行做法：类型筛选 + 名称过滤 + 前端分页）：① 顶部「类型筛选（全部 / 图片 / 视频 / 文本 / 种子 / 其他）+ 名称搜索」工具栏；② 前端分页（每页 60、窗口化页码，仅超一页显示分页条，筛选/搜索变化自动回第一页）；③ 沉浸式 1:1 滑动查看：点击文件进入全屏查看器，每屏一张（图片 fit-contain 且可滚轮/双指捏合/双击缩放与平移、视频默认最大化填充且打开即自动播放、其他类型占位），手指/鼠标跟手横滑翻页，松手按位移阈值吸附到相邻页；下滑或 Esc 退出，顶栏与底部缩略图条一同自动隐藏、交互（移动/点击）时浮现（触摸端跳转需先点一下唤出缩略图条）；底部缩略图条（Filmstrip）横滑浏览「筛选后」的文件（与网格口径一致，翻页不会跳到非筛选类型）、当前项高亮、点击即跳转，视频缩略图取首帧展示（进入视口才懒加载、preload=metadata；桌面端 seek 到 5% 取代表帧，移动端/iOS 走「静音播放→playing 取帧」可靠路径避免黑屏）；媒体按「当前 ±2」窗口懒加载，切换/关闭时自动暂停后台视频（避免切到图片仍出声），播放页媒体带主题蓝(#409eff)边框与投影在纯黑背景上区分当前资源边界，便于连续浏览大量文件。
- **分享接口**（`/api/share`，由独立 8090 分享服务消费）：
  - `POST /api/share`：`{ rel_paths: string[], ttl }` 创建分享链接（rel_paths 为 downloads/ 内相对路径列表，支持单文件或整目录多选；ttl 可选 1h/24h/7d/30d），返回 `{ ok, url, count, name, rels, ttl, expire_at }`；
  - 分享服务（`web/share_server.py`，端口 `TXXY_SHARE_PORT` 默认 8090）：`GET /share/{token}` 画廊式预览页（目录名 + 文件网格 + 类型筛选/名称搜索/前端分页 + 沉浸式 1:1 滑动查看 + 底部缩略图条），`GET /share/{token}/file?i=N` 内联返回第 N 个文件（支持 Range），过期或任一文件已删除返回 410；不与 8088 前端 SPA 共享任何静态资源，天然不暴露管理界面。
- **下载中心接口**（`/api/downloads`）：
  - `GET /api/downloads`：任务列表（概要：状态（`pending/running/paused/done/failed/cancelled`，其中 `paused` 为非终态）、进度、`items_summary` 计数、`saved_dirs`，不含明细）；
  ...
  - `POST /api/downloads/{tid}/cancel`：取消任务——运行中任务标记取消标志（worker 在下一个链接前收手），排队中 / 已暂停任务立即置为已取消终态并使队列令牌失效（不必等 worker 消费）；
  - `POST /api/downloads/batch-start`：**开始下载**（界面「开始下载」按钮与行内「下载」共用，`{ids: [...]}`）——已暂停任务继续未完成链接（不重跑已成功项）、失败/已取消任务在原任务内重跑未成功项，排队中 / 正在下载 / 已完成 / 收尾中的任务跳过；返回 `{ids, links, skipped:[{id, reason}]}`（跳过原因如实回传，前端据此提示）；
  - `POST /api/downloads/batch-pause`：**全部暂停**（`{ids: [...]}`）——排队中任务立即转 `paused` 并使队列令牌失效、下载中任务置暂停标志（在跑链接收尾后停止提交新链接），状态**立即**变 `paused` 且未跑链接保留为 `pending`（非终态，可再「开始下载」继续）；返回结构同 batch-start；
  - `POST /api/downloads/{tid}/prioritize`：排队任务插队（仅 pending 有效；优先级队列 + 令牌失效机制防重复消费）；
  - `POST /api/downloads/clear`：清空「已完成」（done）任务记录，失败/已取消保留（返回 `{cleared}`）；
...
| `docs/数据总览大屏业务优化建议.md` | 业务优化建议（业界做法对照 + 数据可行性 + 分批优先级）；R1-R4 已落地（2026-09-11，详见第十七节） |
