...
- **页面**：
  - 数据总览（分区加载：P0 首屏、P1 视口懒加载 `IntersectionObserver`）：
    ...
    - **采集健康条**（P0）：位于 KPI 卡上方，三态横幅（绿/橙/红）+ 一句话结论（抓取中进度 / 已 N 天无入库 / 最近批次 N 个版块失败 / 入库正常 N/N），由 `/api/stats/health` 提供，判定口径（`run_lag_days` ≥2 升 warn、≥3 升 danger；空窗 vs 抓取中断优先级）收敛在后端 `_health_verdict` 一处，前端只上色不各判一套阈值；悬浮展示批次日期/耗时/版块成功率/最新发布日，点击跳 `/runs`；抓取中时圆点脉冲（与 B1 抓取中徽标同节奏）；
    ...
    - **内容资产漏斗**（P0）：紧凑横条卡四级「收录帖子 → 已下载帖 → 本地文件 → 占用体积」，首级箭头附转化率（已下载帖 / 收录帖子），四级配色沿用趋势统计卡家族色（蓝/绿/紫/橙，不另建色板），由 `/api/stats/assets` 驱动（口径与资源管理页一致：`download_tasks` 中已落盘且文件仍在的 URL 与 posts 取交集）；右侧「资源管理」入口跳 `/resources`；已下载帖与本地文件全为 0 时（任务 + 文件 + 履历全部清空）渲染「尚未下载任何内容」引导空态（含去资源管理入口），而非生硬的「0 漏斗」；
