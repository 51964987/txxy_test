...
- **页面**：
  ...
  - 参数设置（`/settings`）：页内统一配置运行时参数（配置层级 **页内设置 > 环境变量 > 代码默认值**；仅白名单参数，端口/路径/域名等部署配置不在此处）。**下载**：单任务内并行下载数（1-8，下一任务生效）、任务间并行数（1-4，调大立即、调小在当前任务结束后收缩）、单次提交上限、任务历史保留条数、单文件下载间隔、请求重试次数与等待（后三者下一任务生效）；**资源与回收站**：回收站保留天数、资源扫描缓存 TTL（均立即生效）；**界面**：数据总览自动刷新（前端立即应用）；**内容资产**（2026-09-13 新增）：沉淀目标范围（全库 / 互动≥10 / 互动 Top500，单选枚举）与目标覆盖率（%），驱动数据总览资产卡的目标进度条与「还差 N 帖」。每项标注生效范围与默认值，支持单项/全部**恢复默认**；落盘到 `outputs/web_settings.json`（原子写，损坏回落默认），服务启动时自动应用已保存设置；
