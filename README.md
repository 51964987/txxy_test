...
│   ├── api.py          # REST 接口（/api/config、stats、posts、runs、resources、downloads、precipitate/run；统计接口 5s 缓存；/api/schedule 返回 {scrape, precipitate} 两组状态）
...
│   ├── scheduler.py    # 应用内多任务调度（页面可配置：定时抓取 + 定时自动下载，幂等 + 错过不补跑）
│   ├── precipitate.py  # 自动下载后端核心（按设置筛选当天发布的帖子 → 作为 kind="auto" 任务提交下载中心，复用其并发/履历/去重/落盘，与手动下载同一队列）
│   └── frontend/       # Vue3 + Vite + TypeScript + Element Plus + Pinia + ECharts SPA
│       └── src/stores/  # Pinia 状态：app.ts（布局状态：移动断点 / 侧栏折叠 / 抽屉 / 大屏全屏 / 视图重载令牌）+ dashboard.ts（总览 / 实时时钟 / 自动刷新状态）
...
### 4.1 自动下载（定时筛选帖子到下载中心）

把「当天发布」的帖子按页面设置的筛选条件，作为任务提交到下载中心（与手动下载同一队列、同一落盘目录 `downloads/`），复用其全部能力（并发 / 履历 / 去重 / 暂停 / 重跑 / SSE）。

- **触发**：参数设置页「自动下载」组配置开关与时刻（与定时抓取同源的页面可配置调度）。建议自动下载时刻排在抓取时刻之后，确保当天数据已入库。
- **筛选（取交集）**：`date = 当天运行日`（帖子真实发布日）且 `fid ∈ 版块白名单(可空)` 且 `标题含任一关键词(可空)` 且 `(点赞≥阈值 OR 回复≥阈值，阈值默认 0 即不限)`。口径说明：**不取 `update_date`**——它是「最近一次覆盖写入日」，抓取为全站重抓会把历史帖刷成当天，且首次入库帖该列为空（详见 `docs/自动下载设计与实现.md` §3）。
- **落盘**：复用下载中心，媒体与清单落到 `downloads/<sanitize_title(标题)>/`（与手动下载同结构），不单独建目录、不写 `post.json`/`post.md` 元数据。
- **去重**：提交前过滤下载中心已落盘(或正在下载)的同帖，避免空跑与并发写同一文件。
- **磁盘水位守卫**：提交前判定 `downloads/` 所在磁盘可用空间，低于「磁盘水位阈值（GB）」（参数设置页可配，默认 20GB，与资产条「磁盘剩余」红色判据一致）即停止提交，避免写满磁盘；停止原因写入「上次结果」并复用资产条提示。
- **复用**：媒体下载与清单导出直接复用 `download_files.process_one_detail`（图片/视频/种子实下 + 磁力/云盘写清单），仅通过下载中心任务队列落盘，不另写下载逻辑。
- **手动触发**：`POST /api/precipitate/run`（不受定时时刻限制），便于即时验证筛选与落盘。
- 详细设计见 `docs/自动下载设计与实现.md`。
...
- **页面**：
  - 数据总览（分区加载：P0 首屏、P1 视口懒加载 `IntersectionObserver`）：
    - **布局与顶部 Header（全局）**：左侧菜单桌面端可折叠（212px ↔ 64px 图标条，折叠态持久化），<768px 自动切换为抽屉（选中菜单后自动关闭）；**菜单点击保证必有反馈**（2026-09-24）：点击当前所在页 = 重建该视图（刷新），不再因 vue-router 重复导航而静默失败；页面组件懒加载期间顶部显示 2px 进度条；路由 chunk 加载失败（如前端重新构建后旧页面仍开着）自动提示「页面已更新，正在重新加载…」并重载（详见《侧边栏折叠与大屏全屏优化方案.md》第 11 节）；Header 常驻实时时钟（每秒刷新，窄屏隐藏日期段）与「自动刷新」开关（后端 `TXXY_ENABLE_AUTO_REFRESH` 控制，默认开启，5s 静默轮询），两者在大屏全屏态同样保留；数据总览另提供「大屏」按钮，对右侧内容区（Header + 内容）进入浏览器全屏（Esc 退出，不支持元素级全屏的浏览器自动降级为 CSS 满屏），全屏态图表高度按视口三等分并补齐懒加载区块；
  ...
  - 下载中心（`/downloads`）：页内粘贴多行链接直接提交（自动拆解去重、无效行提示，重复提交前经 `check-dup` 接口二次确认），异步执行并以 **SSE 实时推送**进度（亚秒级，断线自动降级 3s 轮询），逐 URL 明细（含耗时）与任务日志在「详情」抽屉按需加载；**任务日志含完整下载过程**（正在请求 / 标题 / 保存目录 / 共提取到 N 个资源 / 逐文件 `[完成] xxx.jpg（字节数,链接）`，与 CLI 看到的一致），明细行以 `[序号/总数]` 前缀标注所属链接，最多保留最近 2000 条；**「按链接明细」折叠组按最后活动时间排序**（最新在最后，而非按链接序号——重下（行内「下载」/单条「重新下载」）只跑未成功链接、其序号往往更小，按序号排会让新输出落在列表中间），正在输出的组标「进行中」，配合自动跟随即 CI 日志的 tail -f 体验；状态筛选与汇总（进行中/已暂停/已完成/失败/已取消）、任务完成通知；任务列表新增「标题」列（按任务 URL 反查帖子标题；一个任务含多标题时显示「主标题 + 等 N 个」，悬浮显示全部标题），任务列表按状态排序（进行中→排队中→已暂停→失败→已取消→已完成，已完成沉底），排队中任务按入队令牌升序展示（与实际执行顺序 FIFO 一致，置顶任务最前）、终态按创建时间倒序，并分页展示（前端切片，每页 20）；任务行可勾选（桌面表格与移动端卡片共用同一选中集合），工具栏**单一主操作按钮随在途状态自动切换**——无在途任务时是「开始下载」（对勾选的失败/已取消/已暂停任务**统一按同一口径**把未完成部分下完：未成功链接在原任务内重跑、已成功的不重复下载；已在队列中/已完成/正在下载的跳过并给出原因），有在途任务时是「全部暂停」（暂停勾选的下载中任务，未勾选则暂停全部在途任务；**暂停是非终态**，未跑链接保留为 pending，可再「开始下载」继续，与「取消」严格区分）；行内「下载」即单任务版的同一入口（不另开任务、进度在原任务更新），排队任务可「优先执行」插队；详情抽屉内链接明细支持**单链接「重新下载」**（就地重跑原任务内该链接，不另开任务，结果仍显示在原详情页），明细区新增成功/失败/取消统计与状态筛选（全部/异常/成功）、异常项优先排序、固定高度内部滚动、分页展示（前端切片，每页 20、序号跨页连续）、「清空已完成」（只删已完成任务，失败/已取消保留；自动轮转仍按全部终态裁剪防膨胀）与历史自动轮转（默认保留 200 条，`TXXY_DOWNLOAD_TASK_MAX_KEEP`）；另有帖子浏览多选批量/单行下载、数据总览热门榜单行下载入口；后端 `download_tasks.py` 任务队列（任务间并发数 `TXXY_DOWNLOAD_TASK_CONCURRENCY` 默认 1 串行，队列支持插队），**任务收尾自动清理本任务遗留的空目录**（仅 `rmdir`，物理上不会误删有内容的目录），状态持久化到 `outputs/download_tasks.json`（`TXXY_DOWNLOAD_TASKS_FILE`，写盘前自动轮转一份 `.bak` 上一代备份，Web 进程不触碰 `posts.db`）；**下载履历**独立持久化到 `outputs/download_history.json`（`TXXY_DOWNLOAD_HISTORY_FILE`，结构 `url → {dir, first_at, checked_at, fail_at, fail_error}`，2026-09-13 由「url → 目录名」升维）：任务列表会被「清空已完成」与自动轮转裁剪，而「哪些帖子已下载过」是持久事实——履历与任务分开留存，判「已下载」= 履历 ∪ 任务历史中目录仍在磁盘者（提交前判重提示、数据总览「待下载推荐 / 内容资产 / 对账」共用同一口径）；新增的 `first_at`（首次落盘时间，只在缺失时写入）供资产增长曲线与沉淀速率使用，`fail_at` / `fail_error` 让**失败清单在任务面板被清空后依然留存**（此前履历只记成功）；每次启动从现存任务播种（`first_at` 取任务结束时刻近似）并按 `sanitize_title(库内标题) == 目录名` 从 `downloads/` 增量恢复（覆盖 CLI 直接下载的目录；**不再用「目录名 == 标题」全等判据**——目录名是标题经非法字符替换 + 截断 80 字符后的结果，长标题会永远匹配不上，2026-09-13 修复后实测认领数 130 → 132、未认领归零），清空任务中心不再丢失「已下载」事实；**顶部 6 张状态统计卡文字单行自适应**（2026-09-19）：每张卡「状态标签 + 数量」两行锁定单行（`white-space: nowrap` + 文字包裹层 `min-width: 0`，数值用等宽数字防位数变化抖动），栅格按侧栏展开的最坏可用宽度折算逐级折行——≥1200px 一行 6 列、767-1199px 3 列、360-559px 2 列（压缩图标/内边距/字号）、≤360px 单列，标签不换行、不溢出、不截断（Playwright 12 视口实测全通过）；
- **定时抓取接口**（`/api/schedule`）：
  - `GET /api/schedule`：调度状态（启用状态 / 计划时刻 / 下次执行 / 今日已处理 / 上次结果与原因 / 调度线程心跳）；触发本身由进程内调度器完成，不对外暴露触发接口（手动触发沿用 `GET /api/runs` 页的 `POST /api/runs/start`）；
...
- **下载中心接口**（`/api/downloads`）：
  ...
  - `POST /api/downloads/batch-start`：**开始下载**（界面「开始下载」按钮与行内「下载」共用，`{ids: [...]}`）——失败 / 已取消 / 已暂停任务**统一**重跑未完成链接（已成功的不重复下载），排队中 / 正在下载 / 已完成 / 收尾中的任务跳过；返回 `{ids, links, skipped:[{id, reason}]}`（跳过原因如实回传，前端据此提示）；
  - `POST /api/downloads/batch-pause`：**全部暂停**（`{ids: [...]}`）——排队中任务立即转 `paused` 并使队列令牌失效、下载中任务置暂停标志（在跑链接收尾后停止提交新链接），状态**立即**变 `paused` 且未跑链接保留为 `pending`（非终态，可再「开始下载」继续；若暂停时已无未跑链接，收尾后直接按明细收敛为终态，不留悬空的「已暂停」）；返回结构同 batch-start；
...
定时抓取由 **Web 服务进程内的调度器**（`web/scheduler.py`）承担，在「参数设置」页的**定时抓取**分组里配置：

| 项 | 默认 | 说明 |
|---|---|---|
| 启用定时抓取 | 开 | 关闭后不触发，也不消费计划时刻 |
| 抓取时刻 | 08:00、20:00 | 每天在这几个时刻各启动一次全量抓取（最多 6 个；保存后立即生效，无需重启） |
| 强制全量重跑（`--restart`） | 开 | 关＝断点续跑（当天已抓过的页跳过） |
| 走本地镜像（1024） | 跟随环境 | 与手动「启动抓取」弹窗的开关同一含义 |

页面同时显示**当前状态（空闲 / 批次 #id 运行中 / 正在启动）/ 下次执行 / 今日已处理 / 上次结果（含跳过与未执行原因）/ 调度线程心跳**，并提供「立即运行一次」用于验证配置（与「运行记录」页的「启动抓取」同一入口）；状态区 **5 秒自动刷新**（页面不可见时暂停），批次在跑时「立即运行一次」自动禁用并给出原因。

行为约定（与业界 cron / systemd timer 口径对齐）：

- **幂等**：判据是「(日期, 计划时刻) 是否已处理过」并落盘 `outputs/scrape_schedule_state.json`，重启不会重复触发；
- **错过不补跑**：晚于计划时刻超过 10 分钟视为「当时服务未运行」，记一次「未执行」（页面可见原因）——需要补跑时点「立即运行一次」；
- **重入保护**：上一批仍在跑（含非本页启动的批次）时跳过本次，不排队堆积；
- **同时只会有一个批次**（两道保险）：页面触发在服务端串行化（检查→拉起→落 pid 同一把锁内，连点无效）；`run_batch.py` 自身还会用 **OS 级文件锁** `outputs/run_batch.lock` 保证单实例——无论从页面、控制台、计划任务还是容器 cron 启动，第二个批次都会打印「跳过」并退出（两个批次同跑会互相抢 1024 镜像、重复写库）；
- **同一时刻只应有一处调度**：本机的 Windows 计划任务 `txxy_daily_batch` 与 Docker `cron` 服务**均已停用**；若要恢复其一，请先在设置页关闭「启用定时抓取」，否则同一天会抓两次（双倍请求源站）。

> 需要 OS 级定时（服务不在也执行）时，仍可用计划任务调 `run_daily.bat`（内容为切到项目根执行 `python run_batch.py --restart`）：
>
> ```bash
...
设计与取舍见 `docs/定时抓取调度调研与建议.md`（含业界对比、实施记录与实测）。
...
| `docs/资源管理页面优化方案.md` | 资源管理页优化：P0/P1 已落地；第八节新增需求（删除软删除、视频播放、回收管理页 `/trash`）已落地；2026-09-19 首屏收敛（H1）与窄屏筛选可达（H2）已落地（第三十九节） |
...
| `docs/侧边栏折叠与大屏全屏优化方案.md` | 侧边栏折叠 / 移动端抽屉、数据总览大屏全屏按钮：现状问题、方案对比与落地说明（已实施）；第 11 节为菜单导航可靠性（2026-09-24 已实施：重复导航静默失败、chunk 加载失败兜底、路由切换进度条） |
