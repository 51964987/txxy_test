...
- **页面**：
  - 数据总览（分区加载：P0 首屏、P1 视口懒加载 `IntersectionObserver`）：
    ...
    - **KPI 统计卡片（6 张）**：累计收录（副指标：近 7 日发布 + 近 7 日环比 `↓xx%` + 覆盖版块数，环比由 `/api/stats/compare.week.delta` 驱动，与活跃榜 7d 滚动窗口同口径）、今日发布（副指标：较昨日环比 + 昨日）、发帖作者（副指标：今日更新人数 + 活跃率）、最近入库（最近批次日期 MM-DD + 批次运行时绿色脉冲「抓取中 N%」徽标 + 中性「更新于 HH:MM」；**停摆/批次失败告警已移交 R1 采集健康条，本卡不再重复呈现**）、最新最热帖（最新数据日互动最高帖的回复数，副指标：版块名 + 赞数，点击下钻该日帖子，来自 `/api/stats/today_top` 首屏 `limit=1`）、媒体文件（本地媒体文件数，副指标：占用体积 + 目录数，点击进资源管理，来自 `/api/stats/assets` 与 R4 内容资产漏斗同源）；由 `/api/stats/overview` + `/api/stats/compare` + `/api/runs` + `/api/stats/today_top` + `/api/stats/assets` 驱动，数据新鲜度以入库活动时间（`latest_run_at`，run_days 最近批次开始/结束时刻较大者）为准；
    - **内容资产漏斗**（P0）：紧凑横条卡四级「收录帖子 → 已下载帖 → 本地文件 → 占用体积」，首级箭头附转化率（已下载帖 / 收录帖子），四级配色沿用趋势统计卡家族色（蓝/绿/紫/橙，不另建色板），由 `/api/stats/assets` 驱动（口径与资源管理页一致：`download_tasks` 中已落盘且文件仍在的 URL 与 posts 取交集）；右侧「资源管理」入口跳 `/resources`；卡内新增「按类型」占比条（图片/视频/种子/文本/其他，按体积占比着色，复用资源管理页同款分类与色板），点击任一类下钻到资源管理页按该类型筛选（继承类型上下文，数字自洽）；已下载帖与本地文件全为 0 时（任务 + 文件 + 履历全部清空）渲染「尚未下载任何内容」引导空态（含去资源管理入口），而非生硬的「0 漏斗」；
