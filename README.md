...
- **页面**：
  ...
  - 帖子浏览（`/posts`）：版块多选 / 日期区间 / 标题或作者关键词筛选，分页排序（发布日期/发布时间/点赞/回复），支持从数据总览下钻（带 `fid` / `author` / `sort` 预筛选，下钻作者回填关键词框）；操作栏图标按钮（悬浮「打开/复制链接/下载」），表格多选 + 一键「批量下载」到下载中心（提交前与下载中心页内提交同一套判重：正在下载中的链接自动剔除避免并发写同一文件，已下载过的先弹窗确认再提交），一键导出 CSV（10 列带 BOM，Excel 可直接打开，限 5 次/分）；
  ...
  - 资源管理：扫描 `downloads/` 目录，按文件夹分组展示文件清单与大小；**全局搜索 / 类型筛选**（跨全部目录，命中片段高亮，命中文件带「所属目录」列，目录名命中可一键展开）；目录按时间 / 名称排序；目录列表**分页展示**（默认每页 30，可切 50/100 或「显示全部」），避免 145+ 个目录一次性铺开；目录头**单行形态**（名称省略 + 文件数·大小·时间 + 类型构成摘要 + 标记）、来源命中时操作区提供**原帖/看帖子**回溯（移动端收进「更多」菜单）、**下载任务关联**标记（跳转下载中心）与「未下载到媒体」空壳提示；所有文件行统一「查看」入口（按类型自动分派：图片大图查看器同列表连看 / 视频弹窗播放同列表连播、拖动进度走 HTTP Range / 文本与种子信息弹窗 / 其他用系统默认程序打开）；目录头「浏览」不展开目录即可网格浏览该目录全部资源（类型筛选 + 文件名过滤 + 前端分页每页 60，图片缩略图懒加载防预览限流，点击卡片走同一查看入口）；文件行「删除」与目录头「删除目录」（**软删除**到 `outputs/trash/`，默认保留 7 天）；工具栏「回收站」抽屉可**恢复**或**彻底删除**（含清空，彻底删除需二次确认）；**容量洞察卡**（类型分布占比 / Top10 目录 / Top10 大文件，两卡片行操作与信息密度对齐：目录行支持浏览/原帖/删除，文件行支持查看/原帖/删除）；「打开」按钮调起资源管理器；搜索/筛选/排序/展开状态会话记忆；批量复制路径；加载失败重试与手动刷新（限流：扫描 60 次/分、图片预览 60 次/分、视频播放 300 次/分、打开目录 10 次/分、删除类 10 次/分）；
